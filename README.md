# Structured Decision Framework (SDF)

A constraint architecture for making complex, high-stakes decisions consistently — regardless of domain, emotional context, or information quality.
...
SDF is a constraint architecture, not a traditional software project. The current repository is documentation-first by design — the framework specification, failure mode record, and domain implementation documentation are the primary artifacts. Runnable code is in development.

The honest reason for the current state: a domain-specific implementation of SDF is actively running in production on a real job search right now. That is live testing of the constraint architecture under real conditions with real consequences. Iterating the framework under those conditions takes priority over building a demo. The failure modes being documented in the active implementation will inform the code architecture when it gets built — the same way every prior version informed the next one.

There is also a more direct argument for why the absence of code is not the absence of proof. A framework built to improve decision-making under complexity should be evaluated on whether it produces better decisions under complexity. The active implementation of SDF is currently being used to navigate a job search across a competitive engineering market — in real time, on real opportunities, with real consequences. Closing that search successfully is a higher-value proof of concept than any test suite. That is the current priority. The code will get built when the framework has finished proving itself on the problem it was designed to solve.

If what you're looking for is a polished open-source library, this isn't that yet. If what you're looking for is evidence that the constraint methodology works across real decision domains with real users, the version history and the domain implementations are that evidence.

Code is coming. The framework is already running.

---

## A Note on AI Access

Running SDF effectively requires access to a large language model. Claude, GPT-4, and Gemini all work — SDF has been validated across all three. Free tiers are sufficient for occasional use but come with limitations: reduced context windows, daily message caps, and memory constraints that can affect longer or more complex evaluations. For consistent, uninterrupted use a paid subscription to at least one platform is recommended.

Current paid options: Claude Pro, ChatGPT Plus, Gemini Advanced. Pricing and tier features change — check each platform directly for current details.
...
You don't need all three. Pick one you're comfortable with and use it consistently. The framework works the same way regardless of which model is underneath it.
...
**Elite** — Maximum investment. Custom output preparation built from the base template with targeted adjustments. All analytical sections run in full. Applied when the input is rare, the output ceiling is consequential, or a named gap exists that targeted framing could honestly close.
...
| Ramp Tier | Definition | Score Impact | Survivability |
|---|---|---|---|
| Days | Closeable before first contact with minimal effort | Minimal — treat near equivalent to partial match | Yes — close it before first contact |
| Weeks | Closeable within a standard 2–4 week cycle with focused effort | Soft partial — small deduction | Possibly — prep immediately |
| Months | Requires sustained project work. Cannot be closed within a standard cycle | Standard gap penalty | Unlikely without adjacent experience |
| Different Domain | Years of specialized experience required. No prep bridges this | Full hard gap penalty | No |
...
**Anchoring:**
When output involves a negotiable value — compensation, terms, scope — anchor at the top of the realistic range on strong-fit inputs. The current baseline is the problem to solve, not the reference point to protect. Let the other side say no to a high anchor rather than preemptively discounting.
...
Not every decision justifies maximum analytical effort. The framework classifies investment level before analysis begins — standard for common inputs, full for high-value inputs, maximum for rare or high-consequence inputs where the output materially changes your trajectory.
...
*Note: SDF development included iterative minor versions between named releases. This changelog documents the architectural changes significant enough to warrant a named version increment. Minor iterations are absorbed into the subsequent named release.*
...
Introduced formal investment tier classification before analysis begins. Three tiers — Standard, Full, and Elite — calibrate analytical depth to the consequence magnitude and rarity of the input. Added Evaluator Probability Statement immediately following fit score — written from the evaluator's perspective assessing likelihood of a favorable result based on output match, competitive pool, visible differentiators, and visible gaps.

*Failure mode identified:* High-consequence inputs were receiving consistent analytical outputs but the framework had no mechanism to verify that the strongest relevant credentials were explicitly surfaced rather than implied.

---

**v2.7 — Within-Category Emphasis Check & Evidence Library**
Added Within-Category Emphasis Check for Elite tier evaluations. Runs after investment classification and before scoring. Identifies the two to three signals the evaluator will look for first, determines whether those signals are explicitly present or only implied in the output template, and flags what needs surfacing. Added Evidence Library as a canonical reference that defines the ceiling of what can be claimed during evaluation. Framing guidance derived from evidence sources must remain truthful to actual capability performed.
...
Two implementations of SDF are in production, each applying the framework architecture to a distinct decision domain. Both are maintained in a private repository.

**JAF — Job Analysis Framework — v2.9**
**CDF — Career Diagnostic Framework — v1.4**

Both version independently through the same failure mode documentation process as SDF itself. Available upon request.

---

## Current Status

SDF is at v2.7 — current stable version. The framework is actively running in production on real decisions across two domain-specific implementations. Full open-source documentation of the framework logic, classification rule templates, weight assignment structure, and output documentation format is complete and available in this repository. Code implementation is in development.
